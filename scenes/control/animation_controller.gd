...
func _on_child_animation_finished(sprite: AnimatedSprite2D):
	...
	var animation_name_ = sprite.animation.split("_tf_")[0]
...
func _on_child_frame_changed(sprite: AnimatedSprite2D):
...
func play(play_value_: PlayValue) -> void:
	...
	var animations_ = _get_animations(
	...
	for child in get_children():
		...
		for animation_ in animations_:
			if child.sprite_frames.has_animation(animation_.animation):
				var start_frame_: int = 0
				
				var transist = _apply_transist(child, animation_.animation)
...
func _apply_transist(sprite_: AnimatedSprite2D, animation_name_: StringName) -> Array:
	var transist_ = [animation_name_, 0]
