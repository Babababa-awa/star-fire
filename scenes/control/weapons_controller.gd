...
func _on_attack_error(_weapon: WeaponUnit, attack_: AttackValue, error_: Core.Error):
	if attack_.meta.has("weapon_controller_use_index"):
		use_error.emit(attack_.meta.weapon_controller_use_index, error_)
	
func _on_attack_before(weapon_: WeaponUnit, attack_: AttackValue):
	if attack_.meta.has("weapon_controller_use_index"):
		signal_can_use = true
...
func _on_attack_after(_weapon: WeaponUnit, attack_: AttackValue):
	if attack_.meta.has("weapon_controller_use_index"):
		use_after.emit(attack_.meta.weapon_controller_use_index)
	
func _on_attack_complete(_weapon: WeaponUnit, attack_: AttackValue):
	if attack_.meta.has("weapon_controller_use_index"):
...
func can_use(use_index_: int) -> bool:
	assert(
		use_index_ >= 0, 
		"Invalid use_inedx_ value. (" + str(use_index_) + ")"
	)
