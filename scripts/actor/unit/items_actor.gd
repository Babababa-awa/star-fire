...
var action_swap: StringName = &"player_item_swap"
...
func can_drop_selected_item() -> bool:
	...
	var level_item_values_ = get_item_area_items()
...
func _can_pick_up_level_item(
	...
	pick_up_mode: Core.ItemMode
) -> bool:
	if level_item_values_.size() == 0:
		return false
		
	if pick_up_mode == Core.ItemMode.MULTIPLE:
		for current_level_item_value_ in level_item_values_:
...
func _use_item(item_value_: ItemValue) -> bool:
	...
	var selected_item_value = get_selected_item()
	
	if item_value_.meta.can_stack:
		if item_value_.meta.count > 0:
			item_value_.meta.count -= 1
		
		if item_value_.meta.count == 0 and item_value_ == selected_item_value:
			remove_selected_item()
	elif item_value_ == selected_item_value:
		remove_selected_item()
...
func drop_selected_item() -> bool:
	...
	var can_drop_ = can_drop_selected_item()
...
func swap_selected_item() -> bool:
	var level_item_values_: Array[LevelItemValue] = get_item_area_items()
	var selected_item_value_ = get_selected_item()
...
func pick_up_item() -> bool:
	var level_item_values_ = get_item_area_items()
	var level_item_value_ = _get_closest_pick_up_level_item(level_item_values_)
		...
		if pick_up_use and can_use_item(level_item_value_.item):
			if use_item(level_item_value_.item):
				if (not level_item_value_.item.meta.can_stack or 
					...
					Core.level.items.remove_item(level_item_value_.item)
				return true
			else:
				return false
...
func pick_up_items() -> void:
	...
	while level_item_values_.size() > 0:
		var level_item_value_ = level_item_values_.back()
...
func _get_closest_pick_up_level_item(level_item_values_: Array[LevelItemValue]) -> LevelItemValue:
	...
	for level_item_value_ in level_item_values_:
		...
		var item_position_ = level_item_value_.node.get_align_global_position(item_alignment)
...
func get_item_area_items() -> Array[LevelItemValue]:
	...
	for level_item_value_ in Core.level.items.get_items_from_area(area_alias_):
		...
		if item_collision_mode == Core.ItemCollisionMode.TILE:
			var unit_position_ = ((unit.get_align_global_position(unit_alignment) + drop_offset) / Core.TILE_SIZE).floor()
			var item_position_ = (level_item_value_.node.get_align_global_position(item_alignment) / Core.TILE_SIZE).floor()
...
func _can_add_item_internal(item_value_: ItemValue, unselected: bool) -> bool:
	for index in items.size():
		if index == selected_slot and unselected:
			continue
			
		var current_item_value_: ItemValue = items[index]
...
func _add_item_internal(item_value_: ItemValue, unselected: bool) -> bool:
	if items[selected_slot] == null:
	...
	for index in items.size():
		...
		if (items[index].alias == item_value_.alias and 
			items[index].meta.can_stack and
			items[index].meta.can_stack_in_items and
			item_value_.meta.can_stack and
			item_value_.meta.can_stack_in_items
		):
			if items[index].meta.count < 0: # Infinite
				return true
				
			if item_value_.meta.count < 0:
				items[index].meta.count = -1
			else:
				items[index].meta.count += item_value_.meta.count
