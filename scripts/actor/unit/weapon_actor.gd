...
var action_use = &"player_weapon_use_"
...
func ready() -> void:
	...
	weapons = unit.get_node_or_null("%Weapons")
	if weapons is WeaponsController:
...
func get_actions() -> Array[StringName]:
	...
	for i in uses:
		actions_.push_back(action_use + str(i + 1))
