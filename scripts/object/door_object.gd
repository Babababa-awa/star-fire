...
func _ready():
	...
	var left_side_area = get_node_or_null("%Area2DLeftSide")
	var right_side_area = get_node_or_null("%Area2DRightSide")
	var vicinity_area = get_node_or_null("%Area2DVicinity")
...
func _can_open_door() -> bool:
	var lock = Core.level.locks.get_lock(lock_alias)
	...
	var lock_state_ = lock.try_unlock()
